...
<head>
<meta charset="UTF-8">
<script src="/java-web-curso/scripts/script.js"></script>
	...
	<tbody>
		<c:forEach var="topico" items="${curso.topicos}">
			<tr>
				...
				<td>
				    ${topico.descricao}
				    <input type="checkbox" name="${topico.id}" class="ckbox">
				</td> 
			</tr>
		</c:forEach>
	</tbody>
...
<form action="RemoverTopico" method="post" id="remover-topico" onsubmit="if(!confirm('Você realmente deseja remover o(s) tópico(s) selecionado(s)?')){return false;}"">
    <input type="hidden" name="idCurso" value="${curso.id}" />
    <input type="submit" value="Remover Tópico" name="remover" id="btn-remover" disabled>
</form>
